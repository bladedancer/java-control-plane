...
curl -sL https://github.com/envoyproxy/envoy/archive/${ENVOY_SHA}.tar.gz | tar xz  --wildcards --no-anchored "*.proto"
mkdir -p "${protodir}/envoy"
cp -r envoy-*/api/envoy/* "${protodir}/envoy"

curl -sL https://github.com/googleapis/googleapis/archive/${GOOGLEAPIS_SHA}.tar.gz | tar xz  --wildcards --no-anchored "*.proto"
...
curl -sL https://github.com/envoyproxy/protoc-gen-validate/archive/v${PGV_VERSION}.tar.gz | tar xz  --wildcards --no-anchored "*.proto"
mkdir -p "${protodir}/validate"
cp -r protoc-gen-validate-*/validate/* "${protodir}/validate"

curl -sL https://github.com/census-instrumentation/opencensus-proto/archive/v${OPENCENSUS_VERSION}.tar.gz | tar xz  --wildcards --no-anchored "*.proto"
mkdir -p "${protodir}/opencensus/proto"
cp -r opencensus-proto-*/src/opencensus/proto/* "${protodir}/opencensus/proto"

curl -sL https://github.com/open-telemetry/opentelemetry-proto/archive/refs/tags/v${OPETELEMETRY_VERSION}.tar.gz | tar xz  --wildcards --no-anchored "*.proto"
mkdir -p "${protodir}/opentelemetry/proto"
cp -r opentelemetry-proto-*/opentelemetry/proto/* "${protodir}/opentelemetry/proto"

curl -sL https://github.com/prometheus/client_model/archive/${PROMETHEUS_SHA}.tar.gz | tar xz  --wildcards --no-anchored "*.proto"
mkdir -p "${protodir}/io/prometheus/client/"
cp client_model-*/io/prometheus/client/metrics.proto "${protodir}/io/prometheus/client/"

curl -sL https://github.com/cncf/xds/archive/${UDPA_SHA}.tar.gz | tar xz  --wildcards --no-anchored "*.proto"
